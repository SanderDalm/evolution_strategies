...
class AdamOptimizer:
    ...
    def compute_update(self, derivative):

        self.t += 1

        self.momentum = self.beta1 * self.momentum + (1 - self.beta1) * derivative
        #self.momentum /= (1-self.beta1 ** self.t)

        #self.rms = self.beta2 * self.rms + (1 - self.beta2) * (derivative * derivative)
